...
contract Child is SwapService, DebtService, Ownable {
        ...
        returns (bool)
    {
        ...
        (uint256 amountIn, uint256 amountOut) = swapToShortToken(
            _shortToken, baseToken, positionReduction, userPositions[_shortToken].backingBaseAmount, baseTokenConversion, baseTokenDecimals, shortTokenDecimals
        );
    ...
    function payOutstandingDebt(
        ...
        if (_paymentToken == _shortToken) {
            ...
            // Swap to short token
            (, uint256 amountOut) = swapExactInput(baseToken, _shortToken, _paymentAmount, baseTokenDecimals, shortTokenDecimals);
